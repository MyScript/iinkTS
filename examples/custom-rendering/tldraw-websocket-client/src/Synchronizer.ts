...
import
{
  TldrawWebSocketClient,
  useTldrawWebSocketClient
} from "./TldrawWebSocketClient"
...
export class Synchronizer
{
  ...
  /**
   * Resolves the shared TldrawWebSocketClient instance and attaches the gestureDetected listener exactly
   ...
   */
  private async getRecognizer(): Promise<TldrawWebSocketClient>
  {
    const serverConfiguration = JSON.parse(window.localStorage.getItem("server") || "{}") as Partial<TServerWebsocketConfiguration>
    // eslint-disable-next-line react-hooks/rules-of-hooks -- useTldrawWebSocketClient is a plain singleton factory, not a React hook
    const recognizer = TldrawWebSocketClient.instance || await useTldrawWebSocketClient(serverConfiguration)
  ...
  protected async determinesContextlessGesture(drawShapes: TLDrawShape[]): Promise<TGesture | undefined>
  {
    ...
    const gestureResult = await TldrawWebSocketClient.instance.recognizeGesture(this.formatDrawShapeToSend(gestureShape))
    if (gestureResult) {
      if (
