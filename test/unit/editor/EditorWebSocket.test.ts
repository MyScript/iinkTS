...
import
{
  ...
  PointerEventGrabber,
  EditorTool,
  TEditorWebsocketOptions
} from "../../../src/iink"
import { WSRecognizerMock } from "../__mocks__/WSRecognizerMock"

describe("EditorWebSocket.ts", () =>
{
  const height = 100, width = 100
  const DefaultEditorWebSocketOptions: TEditorWebsocketOptions = {
    configuration: DefaultEditorWebSocketConfiguration
  }
  ...
  describe("constructor", () =>
  {
    ...
    {
      const editor = new EditorWebSocket(document.createElement("div"), DefaultEditorWebSocketOptions)
      ...
      expect(editor.recognizer instanceof WSRecognizer).toBe(true)
    })
    ...
    test("should instanciate with custom grabber", () =>
    {
      ...
      const customOptions: TEditorWebsocketOptions = {
        configuration: DefaultEditorWebSocketConfiguration,
        //@ts-ignore
        override: { grabber: CustomGrabber }
      }
      //@ts-ignore IIC-1006 Type instantiation is excessively deep and possibly infinite.
      const editor = new EditorWebSocket(document.createElement("div"), customOptions)
      expect(editor).toBeDefined()
      expect(editor.grabber).toBeDefined()
  ...
  describe("drawCurrentStroke", () =>
  {
    ...
    editor.grabber.attach = jest.fn()
    editor.renderer.init = jest.fn()
    editor.recognizer = new WSRecognizerMock()
  ...
  describe("synchronizeModelWithBackend", () =>
  {
    ...
    {
      const options: TEditorWebsocketOptions = JSON.parse(JSON.stringify(DefaultEditorWebSocketOptions))
      options!.configuration!.triggers!.exportContent = "DEMAND"
      const editor = new EditorWebSocket(document.createElement("div"), options)
      editor.grabber.attach = jest.fn()
      editor.renderer.init = jest.fn()
  ...
  describe("Style", () =>
  {
    const customConfig: TEditorWebSocketConfiguration = JSON.parse(JSON.stringify(DefaultEditorWebSocketOptions))
    ...
    const editor = new EditorWebSocket(document.createElement("div"), { configuration: customConfig })
    editor.grabber.attach = jest.fn()
    editor.renderer.init = jest.fn()
    editor.renderer.updatesLayer = jest.fn()
