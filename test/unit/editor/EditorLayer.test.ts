...
describe("EditorLayer.ts", () => {
  describe("editor state badge", () => {
    let layer: EditorLayer

    beforeEach(() => {
      layer = new EditorLayer(document.createElement("div"))
    })

    test("should render a hidden-count badge for online-idle", () => {
      layer.updateEditorState("online-idle", { queuedCount: 0, activeOperations: [] })
      expect(layer.ui.state.root.className).toEqual("editor-state editor-state-online-idle")
    ...
    test("should show the queued count with an explanatory tooltip for syncing", () => {
      layer.updateEditorState("syncing", { queuedCount: 3, activeOperations: [] })
      expect(layer.ui.state.root.className).toEqual("editor-state editor-state-syncing")
    ...
    test("should hide the count when syncing has nothing queued yet", () => {
      layer.updateEditorState("syncing", { queuedCount: 0, activeOperations: [] })
      expect(layer.ui.state.count.style.display).toEqual("none")
    })

    test("should list active operation labels in the tooltip when online-working", () => {
      layer.updateEditorState("online-working", { queuedCount: 0, activeOperations: ["Converting", "Synchronizing"] })
