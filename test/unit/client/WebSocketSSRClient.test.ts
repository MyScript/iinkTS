...
describe("WebSocketSSRClient.ts", () => {
  ...
  const testDatas: { type: TRecognitionTypeV1; config: TWebSocketSSRClientConfiguration }[] = [
    {
      type: "TEXT",
      config: WebSocketSSRClientTextConfiguration as TWebSocketSSRClientConfiguration,
    },
    {
      type: "MATH",
      config: WebSocketSSRClientMathConfiguration as TWebSocketSSRClientConfiguration,
    },
  ]

  test("should instanciate WebSocketSSRClient", () => {
  ...
  describe("init", () => {
    ...
    let wsr: WebSocketSSRClient

    beforeEach(() => {
      wsr = new WebSocketSSRClient(customConf)
    ...
    test("should reject if receive error message", async () => {
      ...
      await expect(promise).rejects.toEqual(new Error(ClientError.WRONG_CREDENTIALS))
      await expect(wsr.event.emitError).toHaveBeenCalledTimes(1)
      await expect(wsr.event.emitError).toHaveBeenCalledWith(new Error(ClientError.WRONG_CREDENTIALS))
    })
  })
  ...
  describe("Ping", () => {
    ...
    let mockServer: ServerWebSocketSSRMock
    beforeEach(() => {
      const url = new WebSocketSSRClient(customConf).url
  ...
  describe("send", () => {
    ...
    test.skip("should reconnect before send message", async () => {
      expect.assertions(1)
      await wsr.init(height, width)
      await wsr.close(1000, "CLOSE_CLIENT")
      const testDataToSend = { type: "test", data: "test-data" }
      await wsr.send(testDataToSend)
      //¯\_(ツ)_/¯  required to wait server received message
      await delay(100)
  ...
  describe("export", () => {
    ...
    testDatas.forEach(({ type, config }) => {
      test(`should send export message for ${type}`, async () => {
        ...
        const my_wsr = new WebSocketSSRClient(customConf)
