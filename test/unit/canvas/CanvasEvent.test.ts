...
import { CanvasEvent, CanvasEventName, TExport, CanvasTool } from "@/iink"

describe("CanvasEvent.ts", () => {
  const element = document.createElement("div")
  const events = new CanvasEvent(element)
  ...
  describe("with custom addEventListener", () => {
    ...
    test("should execute callback on emitToolChanged", () => {
      ...
      events.emitToolChanged(CanvasTool.Write)
      expect(toolCallback).toHaveBeenNthCalledWith(1, CanvasTool.Write)
    })
  ...
  describe("with dom element", () => {
    test("should execute callback on emitLoaded", () => {
      ...
      element.addEventListener(CanvasEventName.LOADED, testCallback)
      events.emitLoaded()
    ...
    test("should execute callback on emitNotif", () => {
      const notif = { message: "Test message", timeout: 42 }
      const testCallback = jest.fn()
      element.addEventListener(CanvasEventName.NOTIF, testCallback)
      events.emitNotif(notif)
    ...
    test("should execute callback on emitError", () => {
      const error = new Error("Error test")
      const testCallback = jest.fn()
      element.addEventListener(CanvasEventName.ERROR, testCallback)
      events.emitError(error)
    ...
    test("should execute callback on emitExported", () => {
      const exports: TExport = { "text/plain": "tatapouet" }
      const testCallback = jest.fn()
      element.addEventListener(CanvasEventName.EXPORTED, testCallback)
      events.emitExported(exports)
    ...
    test("should execute callback on emitChanged", () => {
      ...
      const testCallback = jest.fn()
      element.addEventListener(CanvasEventName.CHANGED, testCallback)
      events.emitChanged(undoRedoContext)
    ...
    test("should execute callback on emitIdle", () => {
      const idle = true
      const testCallback = jest.fn()
      element.addEventListener(CanvasEventName.IDLE, testCallback)
      events.emitIdle(idle)
    ...
    test("should execute callback on emitCleared", () => {
      const testCallback = jest.fn()
      element.addEventListener(CanvasEventName.CLEARED, testCallback)
      events.emitCleared()
      expect(testCallback).toHaveBeenCalledTimes(1)
    })
    ...
    test("should execute callback on emitConverted", () => {
      const exports: TExport = { "text/plain": "tatapouet" }
      const testCallback = jest.fn()
      element.addEventListener(CanvasEventName.CONVERTED, testCallback)
      events.emitConverted(exports)
      expect(testCallback).toHaveBeenNthCalledWith(1, expect.objectContaining({ detail: exports }))
    })
    ...
    test("should execute callback on emitImported", () => {
      ...
      const testCallback = jest.fn()
      element.addEventListener(CanvasEventName.IMPORTED, testCallback)
      events.emitImported(exports)
      expect(testCallback).toHaveBeenNthCalledWith(1, expect.objectContaining({ detail: exports }))
    })
    ...
    test("should execute callback on emitSelected", () => {
      const symbols = [buildIICircle(), buildIIStroke()]
      const testCallback = jest.fn()
      element.addEventListener(CanvasEventName.SELECTED, testCallback)
      events.emitSelected(symbols)
    ...
    test("should execute callback on emitToolChanged", () => {
      const testCallback = jest.fn()
      element.addEventListener(CanvasEventName.TOOL_CHANGED, testCallback)
      events.emitToolChanged(CanvasTool.Write)
      expect(testCallback).toHaveBeenNthCalledWith(1, expect.objectContaining({ detail: CanvasTool.Write }))
    })

    test("should execute callback on emitUIpdated", () => {
      const testCallback = jest.fn()
      element.addEventListener(CanvasEventName.UI_UPDATED, testCallback)
      events.emitUIpdated()
      expect(testCallback).toHaveBeenCalledTimes(1)
    })
    ...
    test("should execute callback on emitSynchronized", () => {
      const testCallback = jest.fn()
      element.addEventListener(CanvasEventName.SYNCHRONIZED, testCallback)
      events.emitSynchronized()
      expect(testCallback).toHaveBeenCalledTimes(1)
    })
  })
