...
describe("IIRecognizedText.ts", () =>
{
  ...
  describe("create", () =>
  {
    ...
    {
      const partial: PartialDeep<IIRecognizedText> = {
      }
      expect(() => IIRecognizedText.create(partial)).toThrow("no strokes")
    })

    test("should create with decorators", () =>
    {
      const pStroke: PartialDeep<IIStroke> = {
        id: "test-id",
        pointers: [
          { p: 1, t: 1, x: 1, y: 1 }
        ]
      }
      const pHighlight: PartialDeep<IIDecorator> = {
      ...
      const symbol = IIRecognizedText.create(partial)
