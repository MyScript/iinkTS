import { Box, IIRecognizedPolyLine, TBox, SymbolType, RecognizedKind } from "../../../../src/symbol"
import { DefaultStyle, TStyle } from "../../../../src/style"
import { PartialDeep } from "../../../../src/utils"
import { buildOIStroke } from "../../helpers"

describe("IIRecognizedPolyLine.ts", () =>
{
  describe("instanciate", () =>
  {
    test("should create with default style", () =>
    {
      ...
      const symbol = new IIRecognizedPolyLine([stroke])
    ...
    test("should create with custom style", () =>
    {
      ...
      const stroke = buildOIStroke()
      const symbol = new IIRecognizedPolyLine([stroke], style)
      expect(symbol).toBeDefined()
      expect(symbol.creationTime).toBeLessThanOrEqual(Date.now())
      expect(symbol.creationTime).toEqual(symbol.modificationDate)
  ...
  describe("overlaps", () =>
  {
    test("should call overlaps of each strokes", () =>
    {
      ...
      const symbol = new IIRecognizedPolyLine([stroke, stroke1])
  ...
  describe("create", () =>
  {
    ...
    {
      const partial: PartialDeep<IIRecognizedPolyLine> = {
      }
      expect(() => IIRecognizedPolyLine.create(partial)).toThrow("no strokes")
    })
    test("should create clone from json", () =>
    {
      ...
      const symbol = new IIRecognizedPolyLine([stroke])
      const json = JSON.parse(JSON.stringify(symbol))
      const newRecognizedArc = IIRecognizedPolyLine.create(json)
