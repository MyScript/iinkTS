...
describe("IIShapeEllipse.ts", () =>
{
  describe("constructor", () =>
  {
    test("should create ", () =>
    {
      ...
      const phi = 0
      const ellipse = new IIShapeEllipse(center, radiusX, radiusY, phi, style)
    ...
    test("should create with default style", () =>
    {
      ...
      const radiusY = 10
      const phi = 0
      const ellipse = new IIShapeEllipse(center, radiusX, radiusY, phi)
  ...
  describe("createBetweenPoints", () =>
  {
    test("should create", () =>
    {
      ...
      const ellipse = IIShapeEllipse.createBetweenPoints(origin, target, style)
      expect(ellipse).toBeDefined()
      expect(ellipse.creationTime).toBeLessThanOrEqual(Date.now())
      expect(ellipse.creationTime).toEqual(ellipse.modificationDate)
      expect(ellipse.style).toEqual(expect.objectContaining(style))
    ...
    test("should create with default style", () =>
    {
      const origin: TPoint = { x: 1, y: 2 }
      const target: TPoint = { x: 4, y: 6 }
      const ellipse = IIShapeEllipse.createBetweenPoints(origin, target)
      expect(ellipse.style).toEqual(DefaultStyle)
    })
  ...
  describe("updateFrupdateBetweenPointsomLine", () =>
  {
    ...
    test("should updateBetweenPoints when target x increas", () =>
    {
      ...
      expect(ellipse.bounds.x).toEqual(1)
      expect(ellipse.bounds.y).toEqual(2)
      IIShapeEllipse.updateBetweenPoints(ellipse, origin, { x: target.x + 6, y: target.y })
    ...
    {
      IIShapeEllipse.updateBetweenPoints(ellipse, origin, { x: target.x, y: target.y + 4 })
