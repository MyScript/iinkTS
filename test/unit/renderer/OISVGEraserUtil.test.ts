import { OIEraser, OISVGEraserUtil } from "../../../src/iink"

describe("OISVGEraserUtil.ts", () =>
{
  const renderer = new OISVGEraserUtil()

  test("should getSVGPath when no pointers", () =>
  {
    const eraserSym = new OIEraser()
    expect(renderer.getSVGPath(eraserSym)).toEqual("")
  })
  ...
  test("should getSVGPath when 2 pointers", () =>
  {
    ...
    expect(renderer.getSVGPath(eraserSym)).toEqual("M 1 1 Q 10 1")
  })
  ...
  test("should getSVGPath when more than 2 pointers", () =>
  {
    ...
    eraserSym.pointers.push({ p: 1, t: 1, x: 10, y: 1 })
    expect(renderer.getSVGPath(eraserSym)).toEqual("M 1 1 Q 5 5 7 5 10 1")
  })
