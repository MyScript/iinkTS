...
describe("OISVGDecoratorUtil.ts", () =>
{
  ...
  describe("getSVGElement", () =>
  {
    ...
    test("should get Highlight", () =>
    {
      ...
      const el = renderer.getSVGElement(decorator, sym) as SVGGeometryElement
    ...
    test("should Highlight to delete", () =>
    {
      const sym = buildOIStroke()
      const decorator = buildOIDecorator(DecoratorKind.Highlight)
      const elNotToDelete = renderer.getSVGElement(decorator, sym) as SVGGeometryElement
      expect(elNotToDelete.getAttribute("opacity")).toEqual(`0.5`)
      sym.deleting = true
      const elToDelete = renderer.getSVGElement(decorator, sym) as SVGGeometryElement
