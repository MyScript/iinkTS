...
import { type IIJiixQueryManager, IIMathVariablePerBlockCanvas, TMathVariable, TMathVariableDefinition } from "@/iink"
...
describe("IIMathVariablePerBlockCanvas.ts", () => {
  ...
  beforeEach(() => {
    ...
    canvas.math.asVariableDefinition = jest.fn().mockResolvedValue(null)
    canvas.math.getVariables = jest.fn().mockResolvedValue([makeVariable()])
    canvas.math.setListVariableValue = jest.fn().mockResolvedValue(undefined)
  ...
  describe("constructor", () => {
    test("should instantiate with canvas and blockIds", () => {
      const component = new IIMathVariablePerBlockCanvas(asCanvas(canvas), ["block-1"])
      expect(component).toBeDefined()
    })

    test("should deduplicate jiixBlockIds", async () => {
      const component = new IIMathVariablePerBlockCanvas(asCanvas(canvas), ["block-1", "block-1", "block-2"])
      await component.show()
      // getVariables called once per unique block
      expect(canvas.math.getVariables).toHaveBeenCalledTimes(2)
    })
  })
  ...
  describe("show()", () => {
    test("should call getVariables and asVariableDefinition for each block", async () => {
      const component = new IIMathVariablePerBlockCanvas(asCanvas(canvas), ["block-1"])
      await component.show()
      expect(canvas.math.asVariableDefinition).toHaveBeenCalledWith("block-1")
    ...
    test("should skip empty jiixBlockId", async () => {
      const component = new IIMathVariablePerBlockCanvas(asCanvas(canvas), ["", "block-1"])
      await component.show()
      expect(canvas.math.getVariables).toHaveBeenCalledTimes(1)
      expect(canvas.math.getVariables).toHaveBeenCalledWith("block-1")
    })
    ...
    test("should set modal title with plural form for multiple blocks", async () => {
      const component = new IIMathVariablePerBlockCanvas(asCanvas(canvas), ["block-1", "block-2"])
      await component.show()
      const allText = document.body.textContent ?? ""
    ...
    test("should mark variable as isDefinition when it matches definition.name", async () => {
      canvas.math.asVariableDefinition = jest.fn().mockResolvedValue(makeDefinition({ name: "x" }))
      canvas.math.getVariables = jest.fn().mockResolvedValue([makeVariable({ name: "x" })])
      const component = new IIMathVariablePerBlockCanvas(asCanvas(canvas), ["block-1"])
      await component.show()
    ...
    test("should render onDelete button for API-typed variable", async () => {
      canvas.math.getVariables = jest.fn().mockResolvedValue([makeVariable({ name: "x", sourceType: "API" })])
      const component = new IIMathVariablePerBlockCanvas(asCanvas(canvas), ["block-1"])
      await component.show()
    ...
    test("should not render onDelete button for BLOCK-typed variable", async () => {
      canvas.math.getVariables = jest.fn().mockResolvedValue([makeVariable({ name: "x", sourceType: "BLOCK" })])
      const component = new IIMathVariablePerBlockCanvas(asCanvas(canvas), ["block-1"])
      await component.show()
      const buttons = Array.from(document.querySelectorAll("button")).filter((b) => b.title === "Delete variable")
    ...
    test("should render sourceLabel for BLOCK-typed variable with sourceId", async () => {
      ...
      const component = new IIMathVariablePerBlockCanvas(asCanvas(canvas), ["block-1"])
      await component.show()
      expect(canvas.jiix.getBlockLabel).toHaveBeenCalledWith("src-block")
      const allText = document.body.textContent ?? ""
  ...
  describe("applyChanges()", () => {
    test("should call setListVariableValue for blocks with changed variable values", async () => {
      canvas.math.getVariables = jest.fn().mockResolvedValue([makeVariable({ name: "x", value: 1 })])
      const component = new IIMathVariablePerBlockCanvas(asCanvas(canvas), ["block-1"])
      await component.show()
      ...
      expect(canvas.math.setListVariableValue).toHaveBeenCalledWith("block-1", { x: 99 })
    })

    test("should not call setListVariableValue when value is unchanged", async () => {
      canvas.math.getVariables = jest.fn().mockResolvedValue([makeVariable({ name: "x", value: 42 })])
      const component = new IIMathVariablePerBlockCanvas(asCanvas(canvas), ["block-1"])
      await component.show()
      ...
      expect(canvas.math.setListVariableValue).not.toHaveBeenCalled()
    })
