const { waitForEditorWebSocket, writeStrokes, waitForExportedEvent, getExportsTypeFromEditorModel, waitEditorIdle } = require("../helper")
...
describe("Websocket Text local storage", () => {
  ...
  test("should export text/plain", async () => {
    const [exports] = await Promise.all([waitForExportedEvent(page), writeStrokes(page, helloOneStroke.strokes)])
  ...
  test("should get hello in localstorage", async () => {
    ...
    await Promise.all([waitForExportedEvent(page), writeStrokes(page, helloOneStroke.strokes)])
