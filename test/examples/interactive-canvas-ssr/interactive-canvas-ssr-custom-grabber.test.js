...
test.describe("Interactive Canvas SSR custom grabber", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(`${process.env.PATH_PREFIX ? process.env.PATH_PREFIX : ""}/examples/interactive-canvas-ssr/interactive_canvas_ssr_custom_grabber.html`)
    await passModalKey(page)
  })

  test("should have title", async ({ page }) => {
    await expect(page).toHaveTitle("Interactive Canvas SSR custom grabber")
  })
  ...
  test("should display alert on right button click", async ({ page }) => {
    ...
    await page.locator("#rootEl").click({ button: "right", position: { x: 100, y: 100} })
