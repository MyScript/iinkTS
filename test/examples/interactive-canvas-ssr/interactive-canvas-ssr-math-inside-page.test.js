...
test.describe("Interactive Canvas SSR Math Inside Page", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(`${process.env.PATH_PREFIX ? process.env.PATH_PREFIX : ""}/examples/interactive-canvas-ssr/interactive_canvas_ssr_math_inside_page.html`)
  ...
  mathContentList.forEach(async (mc) => {
    test(`Math content for ${mc.id}`, async ({ page }) => {
      await test.step(`should open modal canvas`, async () => {
      ...
      await test.step(`should re-open modal canvas with new equation`, async () => {
        await Promise.all([
      ...
      await test.step(`should re-close modal canvas`, async () => {
        await page.locator("#close").click()
        await expect(page.locator("#canvas-modal")).toBeHidden()
      })
    })
