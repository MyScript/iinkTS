...
test.describe("Interactive Canvas SSR Text Customize Stroke Style", () => {

  test.beforeEach(async ({ page }) => {
    await page.goto(`${process.env.PATH_PREFIX ? process.env.PATH_PREFIX : ""}/examples/interactive-canvas-ssr/interactive_canvas_ssr_text_customize_stroke_style.html`)
    await passModalKey(page)
  })

  test("should have title", async ({ page }) => {
    await expect(page).toHaveTitle("Interactive Canvas SSR Text Styling")
  })
  ...
  test("should draw stroke with DefaultTheme", async ({ page }) => {
    ...
    const defaultThemeColor = await page.evaluate("rootEl.iink.theme.ink.color")
