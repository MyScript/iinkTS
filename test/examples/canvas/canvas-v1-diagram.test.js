...
test.describe("Ink Canvas Diagram", () => {

  test.beforeEach(async ({ page }) => {
    await page.goto(`${process.env.PATH_PREFIX ? process.env.PATH_PREFIX : ""}/examples/canvas/canvas_v1_diagram.html`)
    await passModalKey(page)
  })

  test("should have title", async ({ page }) => {
    await expect(page).toHaveTitle("Ink Canvas Diagram")
  })
  ...
  test.describe("Nav actions", () => {
    ...
    test("should undo/redo", async ({ page }) => {
      ...
      let strokes = await rootEl.evaluate((node) => node.iink.model.symbols)
      expect(strokes).toHaveLength(rectangle.strokes.length)
      ...
      strokes = await rootEl.evaluate((node) => node.iink.model.symbols)
      expect(strokes).toHaveLength(rectangle.strokes.length - 1)
