...
test.describe("Ink Canvas Text", () => {

  test.beforeEach(async ({ page }) => {
    await page.goto(`${process.env.PATH_PREFIX ? process.env.PATH_PREFIX : ""}/examples/canvas/canvas_v1_text.html`)
    await passModalKey(page, false)
    await page.locator(".ms-capture-canvas").waitFor({ state: "visible" })
  })

  test("should have title", async ({ page }) => {
    await expect(page).toHaveTitle("Ink Canvas Text")
  })
  ...
  test("Nav actions", async ({ page }) => {
    ...
    await test.step("should undo clear", async () => {
      ...
      expect(await page.locator("#rootEl").evaluate((node) => node.iink.model.symbols)).toHaveLength(1)
    ...
    await test.step("should undo write", async () => {
      ...
      expect(await page.locator("#rootEl").evaluate((node) => node.iink.model.symbols)).toHaveLength(0)
      await expect(page.locator("#result")).toBeEmpty()
    })
