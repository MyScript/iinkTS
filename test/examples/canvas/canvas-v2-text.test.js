...
test.describe("Ink Canvas v2 Text", () => {
  ...
  test("should have title", async ({ page }) => {
    await expect(page).toHaveTitle("Ink Canvas v2 Text")
  })
  ...
  test("Nav actions", async ({ page }) => {
    ...
    await test.step("should undo clear", async () => {
      ...
      expect(await page.locator("#rootEl").evaluate((node) => node.iink.model.strokes)).toHaveLength(1)
      await expect(page.locator("#result")).toHaveText(h.exports["text/plain"][0])
    })

    await test.step("should undo write", async () => {
      await Promise.all([waitForExportedEvent(page), page.locator("#undo").click()])
      expect(await page.locator("#rootEl").evaluate((node) => node.iink.model.strokes)).toHaveLength(0)
      await expect(page.locator("#result")).toBeEmpty()
    })
