...
test.describe('Ink Canvas v2 Raw Content', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(`${process.env.PATH_PREFIX ? process.env.PATH_PREFIX : ""}/examples/canvas/canvas_v2_raw_content.html`)
    await passModalKey(page)
  })

  test('should have title', async ({ page }) => {
    await expect(page).toHaveTitle('Ink Canvas v2 Raw Content')
  })
  ...
  test.describe('Nav actions', () => {
    ...
    test('should undo/redo', async ({ page }) => {
      ...
      expect(await page.evaluate("rootEl.iink.model.strokes")).toHaveLength(rectangleShape.strokes.length)

      await Promise.all([waitForExportedEvent(page), page.locator('#undo').click()])

      expect(await page.evaluate("rootEl.iink.model.strokes")).toHaveLength(rectangleShape.strokes.length - 1)
