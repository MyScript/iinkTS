...
import { callEditorIdle, waitForEditorInit, writeStrokes, waitForSynchronizedEvent, getEditorSymbols, DeferredPromise } from "../helper"
import helloOneStroke from "../__dataset__/helloOneStroke"

test.describe("Offscreen TLDraw", () => {
  // const updatePromise = new DeferredPromise()
  test.beforeEach(async ({ page }) => {
    // page.on('websocket', ws => {
    //   ws.on('framereceived', event => {
    //     if (event.payload.type === "newPart") {
  ...
  test.describe("write", () => {
    ...
    test("should have Export JIIX tab updated", async ({ page }) => {
      ...
      const jiixLoc = await page.locator('.tab-content.active')
      ...
      let ellipsisLoc = await page.getByText('bounding-box')
      await expect(ellipsisLoc).toBeVisible()
      await ellipsisLoc.click();
      ...
      const elemsLoc = await page.getByText('elements')
      await expect(elemsLoc).toBeVisible()
      await elemsLoc.click();
      const elems2Loc = await page.locator('.pushed-content').last()
